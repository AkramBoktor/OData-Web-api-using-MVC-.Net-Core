...
namespace Helpers
{
    public class SqlQueryBuilder
    {
        ...
        {
            if (selectExpandQueryOption.RawSelect == null)
                return "*"; // Select All
        ...
        private string BuildWhereClause(SingleValueNode node)
        {
            ...
            if (left is SingleValueFunctionCallNode left2
                && left2.Parameters.ToList()[0] is SingleValuePropertyAccessNode prop1
                && left2.Parameters.ToList()[1] is ConstantNode val1)
            {
                result += $"{prop1.Property.Name} LIKE '%{val1.LiteralText.Trim(new char[] { (char)39 })}%' ";
            }
            ...
            else if (right is SingleValueFunctionCallNode right2
                    && right2.Parameters.ToList()[0] is SingleValuePropertyAccessNode prop2
                    && right2.Parameters.ToList()[1] is ConstantNode val2)
            {
                result += $" {prop2.Property.Name} LIKE '%{val2.LiteralText.Trim(new char[] { (char)39 })}%' ";
            }
